...
from model_03 import db, User, Post, Comment

app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///mydatabase.db' #'sqlite:///../../mydatabase.db' если в другом корневом каталоге
db.init_app(app)
...
@app.cli.command('init-db')# нужен wsgi.py
def init_db():
    db.create_all()
    print('ok')
...
def add_user():
    ...
    db.session.add(user)# не в саму таблицу
    db.session.commit()# уже подтв добавление в базу
